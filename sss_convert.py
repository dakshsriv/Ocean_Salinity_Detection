...
if var == 'sst':
    ...
    for h in sss_onlyfiles:
        curr_datafile = os.path.join(sss_indir, h)
        print(f'Opening file: {curr_datafile}')

        sss_df = xr.open_dataset(sss_indir + h)
        df = sss_df.to_dataframe()
        sss_dfs = df
...
#        print(sss_df.values.tolist())
        ...
        for x in sss_values: #SST: 0.875, 0.625, 0.375, 0.125 SSS: 0.75, 0.25
            ...
            sss = x[3]
            print(x)
            cur.execute("INSERT INTO salinity (s_date, s_lat, s_lon, s_value) VALUES (?, ?, ?, ?)", (date_, lat, lon, sss))
            """ctr += 1
            if ctr == 100:
                print(x)
                ctr = 0"""
